...
// ---------------------------------------------------------------------------
// Branded email template (Muster military/tactical theme)
      ...
      <td align="center">
        <table role="presentation" width="600" cellpadding="0" cellspacing="0" border="0" style="max-width:600px; width:100%; background-color:${BRAND.cardBg}; border:1px solid ${BRAND.border};">
          ...
          <tr><td style="padding:28px 40px 36px;">
            ...
            <p style="margin:10px 0 0; font-family:Arial,Helvetica,sans-serif; font-size:11px; letter-spacing:2px; color:${BRAND.textMuted}; text-transform:uppercase;">&copy; ${new Date().getFullYear()} Muster</p>
          </td></tr>
        </table>
      </td>
    </tr>
...
// Sender shown in the recipient's inbox. The display name ("Muster") controls
...
const FROM_ADDRESS = `${process.env.FROM_NAME || 'Muster'} <team@tacfit.app>`;
...
export const sendVerificationEmail = async (
  ...
  const emailHtml = renderEmailShell({
    preheader: 'Confirm your email to activate your Muster account.',
    heading: 'Confirm Your Email',
    bodyHtml:
      para(`Welcome to Muster, <strong style="color:${BRAND.heading};">${escapeHtml(username)}</strong>.`) +
      para('Confirm your email address to activate your account, join your team, and start competing.'),
    ctaLabel: 'Verify Email',
    ctaUrl: verificationUrl,
    footerNote:
      'This verification link expires in 24 hours. If you didn\u2019t create a Muster account, you can safely ignore this email.',
  });

  const emailText = `Welcome to Muster, ${username}.\n\nConfirm your email to activate your account:\n${verificationUrl}\n\nThis link expires in 24 hours. If you didn't create a Muster account, you can ignore this email.`;
  ...
  try {
    if (process.env.SENDGRID_API_KEY) {
      await sgMail.send({
        to: email,
        from: FROM_ADDRESS,
        subject: 'Muster - Verify Your Email Address',
        text: emailText,
        html: emailHtml,
...
export const sendPasswordResetEmail = async (
  ...
  const emailHtml = renderEmailShell({
    preheader: 'Reset the password for your Muster account.',
    heading: 'Reset Your Password',
    bodyHtml:
      para(`We received a request to reset the password for your Muster account, <strong style="color:${BRAND.heading};">${escapeHtml(username)}</strong>.`) +
  ...
  const emailText = `Password reset requested for your Muster account, ${username}.\n\nSet a new password here:\n${resetUrl}\n\nThis link expires in 1 hour. If you didn't request this, ignore this email.`;
  ...
  try {
    if (process.env.SENDGRID_API_KEY) {
      await sgMail.send({
        to: email,
        from: FROM_ADDRESS,
        subject: 'Muster - Reset Your Password',
        text: emailText,
        html: emailHtml,
        trackingSettings: TRACKING_SETTINGS,
      });
...
export const sendWelcomeEmail = async (
  ...
  const emailHtml = renderEmailShell({
    preheader: 'Your Muster command access is active. Here\u2019s your mission briefing.',
  ...
  try {
    if (process.env.SENDGRID_API_KEY) {
      await sgMail.send({
        to: email,
        from: FROM_ADDRESS,
        subject: 'Welcome to Muster - Let\'s Get Started!',
        text: emailText,
        html: emailHtml,
        trackingSettings: TRACKING_SETTINGS,
      });
