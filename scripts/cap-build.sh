...
# Copy the Muster Up splash master into the iOS asset catalog so the launch
# screen shows the Muster Up mark instead of the default Capacitor splash.
...
if [ -f "$SPLASH_SRC" ] && [ -d "$SPLASH_DST_DIR" ]; then
  echo "→ Installing Muster Up splash into iOS asset catalog..."
